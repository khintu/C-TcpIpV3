...
int serviceHandler(int msock)
{
	...
	while (1) {
		...
		read_glb_data(gRequestCnt, (void*)&numCount, sizeof numCount);
		++numCount;
		write_glb_data(gRequestCnt, (void*)&numCount, sizeof numCount);
		fprintf(stdout, "%d: Requests serviced counter = %d\n", getpid(), numCount);
	}
	return 0;
}
